...
class AuthController {
  public register = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { email, name, password, dateOfBirth, role, phoneNumber }: RegisterUserInput = req.body;
      ...
      const newUser = new UserModel({ name, email, password, role, phoneNumber });

      if (dateOfBirth) newUser.dateOfBirth = dateOfBirth;

      await newUser.save();
      logger.info(
        `Success in authController.register. New User with email : ${email} has been registered successfully.`
      );
      ...
      logger.error(`Error in authController.register while registering a new user. Error : ${error.message}`);
  ...
  public signout = async (req: Request, res: Response, next: NextFunction) => {
    ...
    req.logout(function (err) {
      ...
      res.status(200).clearCookie("levelup-auth", {
  ...
  public getUserDetail = async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      logger.error(`Error in authController.getUserDetail. Error : ${error.message}`);
      return res.status(400).send({ error: true, message: error.message });
    }
  };
}
